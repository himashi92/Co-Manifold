...
from utils import ramps, losses, test_patch_msd_brats
from dataloaders.msd_brats import BraTS
from utils.dice_loss_brats import EDiceLoss
...
if __name__ == "__main__":
    ...
    model_1 = net_factory_hyperbolic(args, net_type=args.model, in_chns=4, class_num=num_classes - 1, mode="train")
    model_2 = net_factory(args, net_type=args.model, in_chns=4, class_num=num_classes - 1, mode="train")
    ...
    trainloader = DataLoader(db_train, batch_sampler=batch_sampler, num_workers=12, pin_memory=True,
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            loss_seg_dice_1 = criterion(y_prob_1, labeled_gt_set, ce_w=args.ce_w, dl_w=args.dl_w)
            ...
            loss_seg_dice_2 = criterion(y_prob_2, labeled_gt_set, ce_w=args.ce_w, dl_w=args.dl_w)
            loss_elbo2 = 0.5 * (1 + log_sigma2 - mu2 ** 2 - torch.exp(log_sigma2))

            loss_sup_1 = loss_seg_dice_1 + abs(loss_elbo1.mean())
            loss_sup_2 = loss_seg_dice_2 + abs(loss_elbo2.mean())
            ...
            g_critic_segs_1_2 = torch.sigmoid(critic(labeled_gt_set.float()))
            ...
            logging.info(
                'M1 iteration %d : loss : %03f, loss_sup: %03f,loss_space: %03f, loss_mask: %03f, loss_elbo: %03f, best_dice_1: %03f' % (
                    iter_num, loss_1, loss_sup_1, loss_space, loss_mask_1, abs(loss_elbo1.mean()), best_dice_1))
            ...
            g_critic_segs_2_2 = torch.sigmoid(critic(labeled_gt_set.float()))
            ...
            logging.info(
                'M2 iteration %d : loss : %03f, loss_sup: %03f, loss_space: %03f, loss_mask: %03f, loss_elbo: %03f, best_dice_2: %03f' % (
                    iter_num, loss_2, loss_sup_2, loss_space, loss_mask_2, abs(loss_elbo2.mean()), best_dice_2))
            ...
            del loss_1, loss_2, loss_sup_1, loss_sup_2, loss_unsup_1, loss_unsup_2, loss_mask_2, loss_seg_dice_1, loss_seg_dice_2
            gc.collect()
            ...
            if iter_num >= 1500 and iter_num % 150 == 0:
                model_1.eval()

                dice_sample_1 = test_patch_msd_brats.var_all_case(model_1, valloader, patch_size=patch_size,
                ...
                dice_sample_2 = test_patch_msd_brats.var_all_case(model_2, valloader, patch_size=patch_size,
                                                                  save_result=False, test_save_path=None)
