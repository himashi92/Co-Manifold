...
class Encoder(nn.Module):
    def __init__(self, manifold, hidden_dim=300, n_channels=3, n_classes=2, n_filters=16, normalization='none', has_dropout=False,
                 has_residual=False, dataset_name="LA"):
        super(Encoder, self).__init__()
        self.has_dropout = has_dropout
        self.manifold = manifold
        self.dataset_name = dataset_name
        self.hidden_dim = int(hidden_dim / n_channels)
        if self.dataset_name == "LA":
            in_channel_linear = 62720
        else:
            in_channel_linear = 55296
        ...
        self.fc21 = nn.Linear(self.hidden_dim, manifold.coord_dim)
        self.fc22 = nn.Linear(self.hidden_dim, manifold.coord_dim)
    ...
    def forward(self, input):
        ...
        gap_x = self.gap(x5).squeeze(-1).squeeze(-1).squeeze(-1)

        mu = self.fc21(gap_x)
        mu_sp = F.softplus(self.fc22(gap_x)) + Constants.eta
...
class Encoder_H(nn.Module):
    def __init__(self, manifold, hidden_dim=300, n_channels=3, n_classes=2, n_filters=16, normalization='none', has_dropout=False,
        ...
        # self.ls = nn.Linear(in_channel_linear, self.hidden_dim)

        self.gap = nn.AdaptiveAvgPool3d((1, 1, 1))  # define the global average pooling layer
...
class Decoder(nn.Module):
    ...
    def forward(self, features, zs):
        ...
        mu_lin = self.lin(zs)
        v = mu_lin.view(batch_size, -1, 4, 4, 2)
...
class VNet(VAE):
    ...
    def forward(self, input):
        ...
        zs = qz_x.rsample(torch.Size([32]))
        zs_E_H = self.manifold.expmap0(zs)
        out_seg1, feat = self.dec(features, zs)

        return out_seg1, feat, zs, zs_E_H, enc_mu
...
class VNet_H(VAE):
    def __init__(self, params, hidden_dim=256, n_channels=3, n_classes=2, n_filters=16, normalization='none', has_dropout=False,
        ...
        super(VNet_H, self).__init__(
            ...
            dist.Normal,        # likelihood distribution
            eval('Encoder_H')(manifold, hidden_dim, n_channels, n_classes, n_filters, normalization, has_dropout, has_residual, dataset_name),
            eval('Decoder_H')(manifold, hidden_dim, n_channels, n_classes, n_filters, normalization, has_dropout, has_residual, 0, dataset_name),
            params
        )
    ...
    def forward(self, input):
        ...
        qz_x = self.qz_x(*enc_mu)
        zs = qz_x.rsample(torch.Size([32]))
        zs_H_E = self.manifold.logmap0(zs)
        out_seg1, feat = self.dec(features, zs)

        return out_seg1, feat, zs, zs_H_E, enc_mu
