...
def poincare_dist(x, y, *, c=1.0, keepdim=False):
    r"""
    Poincare Distance on the Poincare half plane model
    .. math::
    ...
    """
    c = torch.as_tensor(c).type_as(x)
    return _dist_p(x, y, c, keepdim=keepdim)


def poincare_distance(x, y, *, c=1.0, keepdim=False):
    r"""
    Poincare Distance on the Poincare half plane model
    .. math::
        d_c(x, y) = \frac{2}{\sqrt{c}}\tanh^{-1}(\sqrt{c}\|(-x)\oplus_c y\|_2)
    ...
    """
    c = torch.as_tensor(c).type_as(x)
    return _dist_poincare(x, y, c, keepdim=keepdim)
...
def _dist_p(x, y, c, keepdim: bool = False):
    sqrt_c = c ** 0.5
    dist_c = arccosh(1 + 2 * _mobius_add(-x, y, c).norm(dim=-1, p=2, keepdim=keepdim)**2 / ((1 - x.norm(dim=-1, p=2, keepdim=keepdim)**2) * (1 - y.norm(dim=-1, p=2, keepdim=keepdim)**2)))

    return dist_c * 2 / sqrt_c


def _dist_poincare(x, y, c, keepdim: bool = False):
    sqrt_c = c ** 0.5
    dist_c = arcosh(1 + 2 * _mobius_add(-x, y, c).norm(dim=-1, p=2, keepdim=keepdim)**2 / ((1 - x.norm(dim=-1, p=2, keepdim=keepdim)**2) * (1 - y.norm(dim=-1, p=2, keepdim=keepdim)**2)))
