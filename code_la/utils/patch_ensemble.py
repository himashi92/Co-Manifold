...
def test_uncertainty(model_name, num_outputs, model, image_list, num_classes, patch_size=(112, 112, 80), stride_xy=18,
    ...
    for image_path in loader:
        ...
        uncertainty_map = test_single_case_uncertainty(model, image, stride_xy, stride_z, patch_size,
                                                              num_classes=num_classes)

        if save_result:
            nib.save(nib.Nifti1Image(uncertainty_map.astype(np.float32), np.eye(4)),
                     test_save_path + "%02d_uncer.nii.gz" % ith)
...
def test_all_case(model_name, num_outputs, model, model2, image_list, num_classes, patch_size=(112, 112, 80), stride_xy=18,
                  stride_z=4, save_result=True, test_save_path=None, preproc_fn=None, metric_detail=1, nms=0):
    loader = tqdm(image_list) if not metric_detail else image_list
    ith = 0
    total_metric = 0.0
...
def test_all_case_euclidean(model_name, num_outputs, model, image_list, num_classes, patch_size=(112, 112, 80), stride_xy=18,
    ...
    for image_path in loader:
        ...
        if preproc_fn is not None:
            image = preproc_fn(image)
        prediction, score_map = test_single_case_first_output_model(model, image, stride_xy, stride_z, patch_size,
                                                              num_classes=num_classes)
    ...
    with open(test_save_path + '../{}_performance_euclidean.txt'.format(model_name), 'w') as f:
        f.writelines('average metric of decoder 1 is {} \n'.format(avg_metric))

    return avg_metric


def test_all_case_hyperbolic(model_name, num_outputs, model, image_list, num_classes, patch_size=(112, 112, 80), stride_xy=18,
                  stride_z=4, save_result=True, test_save_path=None, preproc_fn=None, metric_detail=1, nms=0):
    loader = tqdm(image_list) if not metric_detail else image_list
    ith = 0
    total_metric = 0.0
    ...
    with open(test_save_path + '../{}_performance_hyperbolic.txt'.format(model_name), 'w') as f:
        f.writelines('average metric of decoder 1 is {} \n'.format(avg_metric))
...
def test_single_case_first_output(model, image, stride_xy, stride_z, patch_size, num_classes=1):
    ...
    label_map = score_map[0] #(score_map[0] > 0.5).astype(int)
    if add_pad:
...
def test_single_case_first_output_model(model, image, stride_xy, stride_z, patch_size, num_classes=1):
    w, h, d = image.shape
...
def test_single_case_uncertainty(model, image, stride_xy, stride_z, patch_size, num_classes=1):
    w, h, d = image.shape
        ...
        for y in range(0, sy):
            ...
            for z in range(0, sz):
                ...
                with torch.no_grad():
                    y, _, _, _, _ = model(test_patch)
                    curvature = 0.3
                    # cm = torch.linalg.norm(y, dim=-1)
                    # confidence_map = cm.unsqueeze(1)
                    # radius = 1.0 / math.sqrt(curvature)
                    # normalized_cm = confidence_map / radius

                    for i in range(y.size(1)):
                        confidence_map = torch.linalg.norm(y[:, i, ...], dim=0, ord=2)
                        radius = 1.0 / math.sqrt(curvature)
                        normalized_cm = confidence_map / radius

                y = normalized_cm.cpu().data.numpy()
                score_map[:, xs:xs + patch_size[0], ys:ys + patch_size[1], zs:zs + patch_size[2]] \
                    = score_map[:, xs:xs + patch_size[0], ys:ys + patch_size[1], zs:zs + patch_size[2]] + y
                cnt[xs:xs + patch_size[0], ys:ys + patch_size[1], zs:zs + patch_size[2]] \
                    = cnt[xs:xs + patch_size[0], ys:ys + patch_size[1], zs:zs + patch_size[2]] + 1

    label_map = score_map[0].astype(int)
    if add_pad:
        label_map = label_map[wl_pad:wl_pad + w, hl_pad:hl_pad + h, dl_pad:dl_pad + d]
